...
persona_descriptions = {
    ...
    "Sustainability-Focused Investor": "Places strong value on sustainable outcomes and accepts lower returns for higher sustainability quality.",
...
if use_manual_preferences:
    ...
    st.sidebar.info(
        f"{persona}\n\nRisk Aversion: {risk_aversion:.1f}\n\nSustainability Preference: {esg_preference:.3f}"
    )
...
with tab4:
    ...
    st.caption("Summary of how your preferences are being interpreted by the optimiser.")
    ...
    with dash1:
        st.markdown(
            ...
            <div style="{card_style}">
                ...
                <div style="{value_style}">{persona}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with tab5:
    ...
    if sensitivity_choice == "Sustainability Preference":
        x_vals = np.linspace(0.0, 0.12, 60)
        x_label = "Sustainability Preference"
        line_vals = []
        for x in x_vals:
...
The app then layers in extra sustainable-finance features:
